...
execute if score *killer.quantity deathswap.setting matches 1 run item replace entity @a[tag=admin,tag=!creative,scores={deathswap.ui_page=102}] inventory.13 with player_head[custom_data={ui:1b},custom_name={"text":"§eNumber of Killers: One per team"},lore=['§7The winning conditions for each killer are independent and not a team.'],profile={name:"MHF_Steve"},tooltip_display={hidden_components:["profile"]},enchantment_glint_override=true]

#killer.eliminated_alert
execute if score *killer.eliminated_alert deathswap.setting matches 0 run item replace entity @a[tag=admin,tag=!creative,scores={deathswap.ui_page=102}] inventory.12 with paper[custom_data={ui:1b},custom_name={"text":"§cKiller Eliminated Alert: Disabled"}]
