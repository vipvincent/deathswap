...
#killer.quantity
execute if score *killer.quantity deathswap.setting matches 0 run item replace entity @a[tag=admin,tag=!creative,scores={deathswap.ui_page=102}] inventory.13 with player_head[custom_data={ui:1b},custom_name={"text":"§e殺手數量：僅一個"},lore=['§7每位殺手的獲勝條件都是各自獨立的，並非組隊'],profile={name:"MHF_Herobrine"},tooltip_display={hidden_components:["profile"]}]
execute if score *killer.quantity deathswap.setting matches 1 run item replace entity @a[tag=admin,tag=!creative,scores={deathswap.ui_page=102}] inventory.13 with player_head[custom_data={ui:1b},custom_name={"text":"§e殺手數量：每隊一個"},lore=['§7每位殺手的獲勝條件都是各自獨立的，並非組隊'],profile={name:"MHF_Steve"},tooltip_display={hidden_components:["profile"]},enchantment_glint_override=true]

#killer.eliminated_alert
execute if score *killer.eliminated_alert deathswap.setting matches 0 run item replace entity @a[tag=admin,tag=!creative,scores={deathswap.ui_page=102}] inventory.12 with paper[custom_data={ui:1b},custom_name={"text":"§c殺手淘汰提示：關閉"}]
