...
#killer
execute if score mode deathswap.setting matches 1 run item replace entity @a[tag=admin,tag=!creative,scores={deathswap.ui_page=100}] inventory.12 with barrier[custom_data={ui:1b},custom_name={"text":"§c個人戰不支援此設定"}]
...
execute if score mode deathswap.setting matches 2 if score killer deathswap.setting matches 1 run item replace entity @a[tag=admin,tag=!creative,scores={deathswap.ui_page=100}] inventory.21 with written_book[custom_data={ui:1b},custom_name={"text": "§e設定 - 殺手模式"}]
