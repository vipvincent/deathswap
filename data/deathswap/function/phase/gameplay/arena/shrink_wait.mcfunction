#--------------------------------------------------
#Death Swap
#data/deathswap/function/phase/gameplay/arena/shrink_wait.mcfunction
#
#Made by vipvincent
#--------------------------------------------------

#complete shrink
execute if score *arena.shrink deathswap.status matches 1 if score *language deathswap.setting matches 1 run tellraw @a [\
    {storage:"deathswap:ui",nbt:"text.prefix",interpret:true},\
    {text:"The arena has been reduced to "},\
    {score:{name:"*arena.border",objective:"deathswap.status"},color:"yellow"}\
]
execute if score *arena.shrink deathswap.status matches 1 if score *language deathswap.setting matches 2 run tellraw @a [\
    {storage:"deathswap:ui",nbt:"text.prefix",interpret:true},\
    {text:"競技場完成縮圈至"},\
    {score:{name:"*arena.border",objective:"deathswap.status"},color:"yellow"}\
]
...
#shrink end 
execute if score *arena.border deathswap.status matches ..50 run return run function deathswap:phase/gameplay/arena/shrink_end
